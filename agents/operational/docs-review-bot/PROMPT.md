...
7. **Diátaxis hub drift (`[MAJOR]`):** the most recent commit adds, renames, or removes a file anywhere under `docs/` — including subdirectories like `docs/how-to/`, `docs/tutorials/`, and `docs/adr/` — **without** also touching [`docs/README.md`](../../../docs/README.md). Exclusions (these directories grow on their own internal indices, not the hub): `docs/archive/`, `docs/daily-reviews/`, `docs/postmortems/`, `docs/issues/`, `docs/plans/`, `docs/superpowers/`, `docs/steering/`, `docs/scripts/` (regenerable TypeDoc), `docs/glossary/` (one-file-per-term per [ADR-0010](../../../docs/adr/0010-shard-glossary-into-one-file-per-term.md)). The rule is in [`CONTRIBUTING.md`](../../../CONTRIBUTING.md); the hub is the index of record. Edits to existing in-scope files (no add / rename / remove) do not trigger this finding.
8. **Tutorial drift (`[BLOCKER]`):** the most recent commit adds, removes, or renames a `/spec:*` slash command (file under `.claude/commands/spec/`), or changes the artifact a stage produces (the per-stage subagent under `.claude/agents/`). The first-feature tutorial at [`docs/tutorials/first-feature.md`](../../../docs/tutorials/first-feature.md) makes load-bearing assumptions about command names and per-stage outputs. Flag the tutorial for re-run from a clean clone of `develop`; if the tutorial has not been re-validated within 90 days of the change, escalate to `[BLOCKER]`.
