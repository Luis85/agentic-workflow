---
description: Design Track — Phase 4 (Handoff). Invokes design-lead to sequence ui-designer and produce design-handoff.md, the gate artifact that leaves the track.
argument-hint: [design-slug]
allowed-tools: [Agent, Read, Edit, Write]
model: sonnet
---

# /design:handoff

Run **Phase 4 — Handoff** of the Design Track. Read [`docs/design-track.md`](../../../docs/design-track.md) §3.4.

1. Resolve the slug from `$1` or by inspecting `designs/` for the active design at `current_phase: handoff`.
2. Confirm `designs/<slug>/sketch.md` and Phase 3 mock decisions exist; if not, route to `/design:mock` first.
3. **Spawn the `design-lead` subagent** with the slug. The design-lead will:
   - Sequence `ui-designer` to finalise component assignments, token references, microcopy, and accessibility checklist.
   - Produce `designs/<slug>/design-handoff.md` from [`templates/design-handoff-template.md`](../../../templates/design-handoff-template.md), synthesising brief → sketch → mock → final decisions.
   - Run the quality gate.
4. Update `design-state.md`: mark `design-handoff.md: complete`, set `status: done`.
5. **Block on human approval** of the handoff before recommending downstream work (`/spec:design` for feature implementation against the new surface, or direct engineering pickup).

## Don't

- Don't ship a handoff with unresolved "TBD" microcopy.
- Don't skip the accessibility checklist.
- Don't proceed without human approval — handoff is the gate that leaves the track.
