...
- **`.specorator-version` content trust:** the file is plain text written by the conductor; its content is the template SHA + ISO date, both controlled by the conductor at Phase 4 time. The idempotency check parses the file with a strict regex; any unparseable content surfaces the collision flow rather than trusting the value (REQ-ADOPT-014, RISK-ADOPT-002).
