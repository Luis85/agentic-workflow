...
- **21 open clarifications remaining.** CLAR-ADOPT-018 and CLAR-ADOPT-020 resolved (2026-05-03). Stage 4 (Design) is now unblocked. Items that block Stage 5: 004 (idempotency marker), 005 (agent enforcement), 006 (preset manifest), 008 (patch-file fallback), 009 (branch collision), 012 (phase success definition), 013 (slug truncation), 014 (resumption scope), 016 (conflict comparison semantics), 017 (manual-merge clearing), 021 (SSH remote guard), 022 (commit timing), 024 (parity validation script).
