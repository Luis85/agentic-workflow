...
| Investigate adding a "release-tag pending" sub-state to Stage 10 (or a `release_tag_cut: false` flag in `workflow-state.md` frontmatter) to distinguish release-readiness-complete from release-complete. Raise as an ADR if it requires a schema change. | adr | architect | 2026-07-01; [#216](https://github.com/Luis85/agentic-workflow/issues/216) |
