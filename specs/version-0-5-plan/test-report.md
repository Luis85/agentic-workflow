...
**Resolution (2026-05-02, dev, T-V05-013, round-6):** Round-6 fixes Codex round-5 P2 on PR #202 (commit `a9adc3a`). Codex caught a deeper issue: `package.json#files` whitelists whole directories, so any `--no-clean` mode silently re-ships stale files from prior runs. **Fix:** dropped `--no-clean` entirely. CLI always cleans. `AssertSafeOutDirOptions.destructive` reverted from the lib. 4 round-5 tests for the non-destructive branch removed. The round-4 P2 fix (round-5 commit) was the wrong layer — should have been "remove the flag" not "fix the marker check". Test count 261. Verify green.

**Resolution (2026-05-02, dev, T-V05-013, round-5):** Round-5 fixes Codex round-4 P2 on PR #202 (commit `4b4d009`). Round-2 + round-4 left `--no-clean` broken: `assertSafeOutDir` ran unconditionally, the marker / non-empty rejection fired even when the build was about to preserve contents, and the rejection message recommended `--no-clean` when that flag was already set (self-contradictory). **Fix:** added `AssertSafeOutDirOptions.destructive` (default `true`). When `destructive: false` only absolute-path guards run; the `.git`-entry and non-empty-without-marker rejections are skipped because `--no-clean` preserves contents. CLI passes `destructive: parsed.cleanFirst`. 4 new tests; total 263. Verify green. Plus merged `origin/main` (10 PRs since round-1) — conflicts resolved per `feedback_parallel_pr_conflicts.md` (merge not rebase).

**Resolution (2026-05-02, dev, T-V05-013, round-4):** Round-4 fixes Codex round-3 P1 on PR #202 (commit `208a200`). The round-2 prepack guard would have blocked the manual recovery runbook in `docs/release-operator-guide.md` §7.1 (bare `npm pack --silent` and `npm publish` from the repo root). **Fix:** §7.1 + §9 quick-reference now use `npm run build:release-archive && npm pack ./.release-staging` and `npm publish "${TARBALL}"`, byte-equivalent to workflow step 5 + step 10. The runbook was already misleading even before round-2 — bare `npm pack` would have shipped 22 numbered ADRs against ADR-0021 / SPEC-V05-010 — the prepack guard exposes that inconsistency rather than introducing it. Test count unchanged (259); `npm run verify` green. Round-4 reaches the soft loop cap; cap-reached comment posted to PR #202.

**Resolution (2026-05-02, dev, T-V05-013, round-3):** Round-3 fixes Codex round-2 P1 on PR #202 (commit `b13a57e`). Round-2's prepack guard introduced a chicken-and-egg dependency: `enumerateFilesViaNpmPack` invoked `npm pack --dry-run --json` from the repo root, which still runs `prepack` (only `--ignore-scripts` skips lifecycle scripts), and the guard intentionally fails from the repo root. Without a fix, `npm run build:release-archive` would always exit 1, breaking release workflow step 5. **Fix:** added `--ignore-scripts` to the inner `npm pack --dry-run --json` invocation only. Bare `npm pack` from the repo root (no flag) still fails closed via the guard, so the round-2 defense is preserved. Verified locally end-to-end: build CLI produces an 826-file staged tree; `npm pack ./.release-staging` (marker present in staged dir) succeeds; Layer 2 `check:release-package-contents` against the extracted candidate exits 0 with zero diagnostics; bare `npm pack` from repo root errors with the guard's helpful message pointing at `npm run build:release-archive && npm pack ./.release-staging`.
