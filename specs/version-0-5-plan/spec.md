...
- **Inputs:** the path lists enumerated in ADR-0021 — the 10 intake folders (`inputs/`, `specs/`, `discovery/`, `projects/`, `portfolio/`, `roadmaps/`, `quality/`, `scaffolding/`, `stock-taking/`, `sales/`) and the ADR file glob `docs/adr/0\d{3}-*.md`.
- **Behavior:** the contract is three deterministic assertions evaluated against the candidate published archive before publish:
  1. **No ADR files ship.** No file matching `docs/adr/0\d{3}-*.md` is present in the published archive. `docs/adr/README.md` **does** ship in the archive, rewritten as a stub per `templates/release-package-stub.md` (it explains to the consumer how to file their own ADRs starting at `ADR-0001`). `templates/adr-template.md` ships unchanged.
