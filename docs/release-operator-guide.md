...
5. **Repo Settings → General → Releases → "Immutable releases" is DISABLED.** When the setting is on, GitHub auto-flags every new Release immutable. If asset upload then fails — or the operator deletes the Release — the tag is **permanently burned**: the GitHub API returns HTTP 422 `tag_name was used by an immutable release` to every later attempt to host a Release on that tag, including a fresh draft. The v0.5.0 publish dispatch hit exactly this and forced the v0.5.1 recovery release ([#233](https://github.com/Luis85/agentic-workflow/issues/233); incident timeline in `specs/version-0-5-plan/retrospective.md` §Incident).

If any of items 1–4 is missing, **stop**. The readiness check will fail closed (preferred). Item 5 is *not* enforced by the readiness check today — the GitHub `/repos/{owner}/{repo}` API does not surface the Immutable Releases setting (UI-only beta) — so the operator owns this pre-condition. Verify it by hand before every dispatch until prevention E from #233 lands a probe via the most-recent-Release `immutable` flag.
...
| Asset upload failed against a Release that GitHub auto-flagged immutable (Repo Setting → "Immutable releases" was on). | **Do NOT delete the Release.** Either attach the asset manually via drag-drop on the Release page in the UI (UI may bypass the API immutability check), or accept an asset-less Release and rely on `npm install @luis85/agentic-workflow@X.Y.Z` as the consumer-facing artifact. Then disable the Immutable Releases setting before the next dispatch. | Deleting an immutable Release **permanently burns the tag**: every later attempt to host a Release on that tag returns HTTP 422 `tag_name was used by an immutable release`. The v0.5.0 incident burned `v0.5.0` and forced the v0.5.1 recovery release ([#233](https://github.com/Luis85/agentic-workflow/issues/233)). |
...
1. **Attach the asset via the GitHub UI.** Navigate to the Release page, drag the candidate tarball (the byte-equivalent output of `npm run build:release-archive` followed by `npm pack ./.release-staging`, see §7.1 step 2 for the recipe) onto the Release. The UI sometimes bypasses the API's immutability check. If it succeeds, the Release is complete and the dispatch counts as recovered.
2. **Accept an asset-less Release.** The npm package on GitHub Packages remains the consumer-facing artifact (`npm install --save-dev @luis85/agentic-workflow@X.Y.Z`); document the missing asset in `specs/version-X-Y-plan/release-notes.md` §Known limitations and move on. The Release page still exists, the tag is still cut, the package is still installable.
3. **Ship a recovery release.** If neither (1) nor (2) is acceptable — for example because the Release page is the consumer-facing artifact — bump to `vX.Y.(Z+1)`, restate the v0.5 incident pattern in the new release notes, disable the Immutable Releases setting **before** the new dispatch, and run the standard publish path. The original burned tag stays burned forever; only the new tag carries a stable Release. This is the path the v0.5.1 recovery release took.

After any of (1), (2), or (3): disable the Immutable Releases setting if it is still on, update §1 pre-conditions in your operator runbook, and file the incident in the project's retrospective and `#233` punch list.
