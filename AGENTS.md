...
- **Template-self changes route to `/specorator:update`.** Any change touching `docs/`, `.claude/`, `scripts/`, `templates/`, `memory/`, `AGENTS.md`, or `CLAUDE.md` that isn't a downstream feature delivery must use the Specorator Improvement Track before any generic brainstorming, planning, or implementation skill fires. Keywords that should trigger this guard: new track, new agent, new skill, new workflow, update template, change workflow, add script, add CI. Artifacts land in `specs/<feature>/` with REQ/T IDs, not in ad-hoc locations.
- **No direct commits on `main` / `develop`.** Every change lands via topic branch + merged PR. Push deny is the backstop, not the gate. A PreToolUse hook in `.claude/settings.json` also blocks `git commit` when `HEAD` points at `main` or `develop`. See [`.claude/memory/feedback_no_main_commits.md`](.claude/memory/feedback_no_main_commits.md).
