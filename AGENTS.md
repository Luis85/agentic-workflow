...
- **Lifecycle agents** (`.claude/agents/`) — used to build one feature across Stages 1–11. The `orchestrator` routes between them. Three sub-classes:
  ...
  - **Stock-taking specialist** *(pre-Discovery or pre-Stage 1, opt-in for legacy/brownfield projects — see [ADR-0006](docs/adr/0006-add-stock-taking-track-for-legacy-projects.md))* — `legacy-auditor`. Used to inventory existing systems (processes, use-cases, integrations, data, technical debt) before ideation or feature work begins. Produces `stock-taking-inventory.md` which feeds the Discovery Track or `/spec:idea`. The stock-taking methodology lives at [`docs/stock-taking-track.md`](docs/stock-taking-track.md).
...
Skills (`.claude/skills/`) are reusable how-tos any agent can invoke (`verify`, `new-adr`, `review-fix`). Three workflow-conductor skills (`orchestrate`, `discovery-sprint`, `stock-taking`) provide the conversational entry points.
