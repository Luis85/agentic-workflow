 ...
 */
export function renderCommandInventory({ fenced = false }: { fenced?: boolean } = {}): string {
  const groups = new Map<string, CommandRecord[]>();
  for (const command of getCommands()) {
    const group = groups.get(command.namespace) || [];
    group.push(command);
    groups.set(command.namespace, group);
  }
  const sections = [];
...
function wrapCommands(commands: string[]): string {
