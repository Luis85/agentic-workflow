 ...
 */
export function getAdrs(): AdrRecord[] {
 ...
 */
export function renderAdrIndex(): string {
  const rows = [
    "| # | Title | Status |",
    "|---|---|---|",
    ...getAdrs().map(
...
function titleFromHeading(text: string): string | null {
  const match = text.match(/^#\s+ADR-\d{4}\s+—\s+(.+)$/m);
  return match ? match[1].trim() : null;
}

function statusFromBody(text: string): string | null {
