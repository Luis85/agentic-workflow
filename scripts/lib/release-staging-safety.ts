...
export function assertSafeOutDir(outDir: string, repoRoot: string): void {
  ...
  throw new Error(
    `release-staging-safety: refusing to clean \`${resolvedOut}\` — non-empty and missing staging marker (\`${STAGING_MARKER_FILE}\`). Choose a fresh dir or remove the contents manually.`,
  );
}
