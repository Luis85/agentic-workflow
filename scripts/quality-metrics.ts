...
const trend = baseline ? compareQualityMetrics(metrics, baseline.metrics, baseline.path) : null;
const savedPath = save ? saveQualityMetricsSnapshot(metrics) : undefined;

if (json) {
  if (compare || save) {
    console.log(JSON.stringify({ metrics, trend, savedPath }, null, 2));
  } else {
    console.log(JSON.stringify(metrics, null, 2));
  }
} else {
  const output = [renderQualityMetrics(metrics).trimEnd()];
  if (compare) output.push(renderQualityTrend(trend).trimEnd());
